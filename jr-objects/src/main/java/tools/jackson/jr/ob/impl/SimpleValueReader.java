...
import static tools.jackson.jr.ob.impl.ValueWriterLocator.*;
...
//        case SER_OBJECT_ARRAY:
    ...
    protected long[] _readLongArray(JsonParser p) throws JacksonException {
        if (JsonToken.START_ARRAY.equals(p.currentToken())) {
            p.nextToken();
        }
    ...
    protected boolean[] _readBooleanArray(JsonParser p) throws JacksonException {
        if (JsonToken.START_ARRAY.equals(p.currentToken())) {
            p.nextToken();
        }
    ...
    protected short[] _readShortArray(JsonParser p) throws JacksonException {
        if (JsonToken.START_ARRAY.equals(p.currentToken())) {
            p.nextToken();
        }
    ...
    protected float[] _readFloatArray(JsonParser p) throws JacksonException {
        if (JsonToken.START_ARRAY.equals(p.currentToken())) {
            p.nextToken();
        }
